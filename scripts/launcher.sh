...
if [[ "${FD_CONTAINER_DEBUG_SCRIPTS}" ]]; then
    debug=debug_print
else
    debug=true
fi
...
while [[ $# -gt 0 ]]; do
    case "${1}" in 
        "--fd_singularity_overlay_path_override")
            ### Sometimes we do not want to use the 'correct' container
            export CONTAINER_OVERLAY_PATH_OVERRIDE=1
            $debug "fd_singularity_overlay_path_override=1"
            shift
            ;;
        "--fd_singularity_overlay_path")
            ### From time to time we need to manually specify an overlay filesystem, handle that here:
            export CONTAINER_OVERLAY_PATH="${2}"
            $debug "fd_singularity_overlay_path="${CONTAINER_OVERLAY_PATH}
            shift 2
            ;;
        "--fd_singularity_in_container_path")
            ### Set path manually
            export PATH="${2}"
            $debug "fd_singularity_in_container_path="${PATH}
            shift 2
            ;;
        "--fd_singularity_launcher_override")
            ### Override the launcher script name
            export LAUNCHER_SCRIPT="${2}"
            $debug "fd_singularity_launcher_override="${LAUNCHER_SCRIPT}
            shift 2
            ;;
        "--fd_singularity_singularity_path")
