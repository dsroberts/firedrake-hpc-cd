...
pip3 install --target "${HPC_HELPER_PATH}" --upgrade --no-deps git+https://github.com/g-adopt/gadopt_hpc_helper
### Replace interpreter line in gadopt_hpcrun
export PY3_WRAPPER=$( type -p python3.11 )
sed -i  '1 s:^.*$:'#\!${PY3_WRAPPER}':' "${HPC_HELPER_PATH}/bin/gadopt_hpcrun"
