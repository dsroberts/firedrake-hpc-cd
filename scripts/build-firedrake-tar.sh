...
for inplace in "-inplace" ""; do
    ...
    export FIREDRAKE_USE_BUILD_OVERRIDE="${inplace}"
    ls "${MODULE_PREFIX}/petsc${APP_BUILD_TAG}"
    ### 3.) Load dependent modules
    if [[ -d "${MODULE_PREFIX}/petsc${APP_BUILD_TAG}" ]]; then
    ...
    echo Building against $PETSC_MODULE
    echo $PETSC_TAG
    echo $PETSC_DIR_SUFFIX


    export APP_BUILD_TAG="${inplace}${APP_BUILD_TAG}"
    ...
    if [[ "${inplace}" ]]; then
        mkdir "${APP_IN_CONTAINER_PATH}/${TAG}"
        if [[ ! -d "${APP_IN_CONTAINER_PATH}/${TAG}${VERSION_TAG}" ]]; then
            tar -xf "${BUILD_STAGE_DIR}/${APP_NAME}.tar" -C "${APP_IN_CONTAINER_PATH}/${TAG}"
        fi
        export PETSC_DIR="${APPS_PREFIX}/petsc${APP_BUILD_TAG}/${PETSC_DIR_SUFFIX}${VERSION_TAG}"
    else
        if [[ ! -d "${APP_REAL_INSTALL_DIR}/${BUILD_DIR_NAME}" ]]; then
            mkdir -p "${APP_REAL_INSTALL_DIR}/${BUILD_DIR_NAME}"
        ...
        tar -xf "${PETSC_DIR}.tar" -C "${APP_REAL_INSTALL_DIR}"
    fi
    ...
    ### Expected by function, not used elsewhere
    export SQUASHFS_PATH="${APP_IN_CONTAINER_PATH}"
    export SQUASHFS_APP_DIR="${TAG}"
    ...
    fix_apps_perms "${APP_IN_CONTAINER_PATH}"
    deactivate
